<!-- Canonical copy: /root/AGENTS.md — edits there, then re-sync. Synced 2026-09-20 (overnight: L6 ACSOAP realtime transport + GO/chest looting [VisitGridObjects fix] + level-1 regen + idle-sweep removal; commit 54feac8f). -->
...
### Quest Accept/Turn-in Pipeline (PROVEN end-to-end, 2026-09-19, commit 352871d6)
- **Proven:** full accept→turnin via the agent file queue (Inisto, Night Elf lvl 59, quest 16 "Give Gerard a Drink"): `rms wander,grind,loot` (stand still) → `teleport -9919 39 33 0` (land dist 0.0 on giver 255 "Gerard Tiller", guid 80686) → `interact 80686` (menu `16(icon4)` = available/auto-complete) → `accept 16 80686` (accepted) → `turnin 16 80686` (rewarded + removed from quest log; log `{}` after).
- **TURN-IN OPCODE FLOW (critical — NOT `COMPLETE_QUEST`):** the real turn-in is TWO opcodes replayed through the session handlers: (1) `CMSG_QUESTGIVER_REQUEST_REWARD` → `HandleQuestgiverRequestRewardOpcode` = `CompleteQuest` (status→COMPLETE) + offer-reward dialog; (2) `CMSG_QUESTGIVER_CHOOSE_REWARD` (packets: giverGuid, questId, chooseReward=0) → `HandleQuestgiverChooseRewardOpcode` = `RewardQuest` (actual reward grant + `RemoveQuest` from log). `CMSG_QUESTGIVER_COMPLETE_QUEST` is ONLY the hand-in-items dialog (NO state change) — using it for turn-in does nothing. `accept` = `CMSG_QUESTGIVER_ACCEPT_QUEST` → `HandleQuestgiverAcceptQuestOpcode` = `AddQuest`; auto-complete quests (IsAutoComplete) go COMPLETE on accept → turn-in directly.
- **DB facts (this core):** `creature_template` PK = `entry` (NOT `id`); `creature` spawn table PK = `id` (NOT `entry`), no `name` col; `creature_questrelation`/`creature_involvedrelation` cols = `id` + `quest`; `UNIT_NPC_FLAG_QUESTGIVER = 2` (UnitDefines.h) — **NOT 8** (8=FLIGHTMASTER); quest-giver scans must use `npc_flags & 2`. `quest_template` custom cols: `RequiredClasses` (bitmask bit(class), class 0-based), `RewChoiceItemId1-6`, `RewItemId1-4` (NOT vanilla names). `characters` has NO `exp`/`experience` col (only `level`).
- **Enums:** `QuestStatus` NONE=0/COMPLETE=1/INCOMPLETE=3/AVAILABLE=4; `character_queststatus.status=1 rewarded=0` = complete-not-yet-rewarded (pre-turnin). `DIALOG_STATUS_AVAILABLE=5`, `DIALOG_STATUS_REWARD_REP=4` (icon4 = available/auto-complete, not "unavailable").
- **Agent commands added (352871d6):** `strategies [state]`, `adds/rms/toggles <names> [state]` (AC `+`/`~`/`-`, comma lists flattened per name), `clear [all|state]`, `teleport <x> <y> <z> [map]` (ground-height `INVALID_HEIGHT` + `IsValidMapCoord` guards). Engine::ChangeStrategy/ToggleStrategy/RemoveAllStrategies/ListStrategies (AC pattern) + PlayerbotAI passthrough.
- **Test gotcha:** a low-level bot dies to the Elwynn-giver mobs (lvl-2 → dead in ~10s, engine "dead"). Use a HIGH-level bot (lvl 59; quest 16 MaxLevel 0 = uncapped) and land EXACTLY on the giver (dist 0.0; nearby boars ~23yd don't aggro the spot).
- **Relocation sweep:** `PlayerBot.RelocateIdleEnabled=0` set for the test (so a strategy-less bot stands still). Travel (1-5h) remains the fallback self-heal. PENDING decision: keep off (AC-purist; L2 walking is the proper fix) vs re-enable.

### R7 Walking Movement (L1) + RPG State Machine (L2) + Quest Data (L3) (2026-09-19)
- **Mission (user directive 2026-09-19): 100% port of azerothcore mod-playerbots — no invented behavior.** AC bots **WALK** to givers/POIs/towns via engine pathfinding; teleport is stuck-recovery ONLY. The teleport-to-POI sweep + 180s idle-relocation were INVENTED band-aids (rejected). Full R7 design + AC mining in `bot-master-plan.md` §R7.
- **L1 (commit 21279af2, DONE+deployed):** `MovementActions` = full AC MovementAction protected API — `MoveTo/MoveNear/MoveDelay/WaitForReach/IsWaitingForLastMove/IsMovingAllowed/SearchForBestPath/DoMovePoint` (`mm->Clear()`+`MovePoint(MOVE_PATHFINDING)`), `MoveFarTo` (AC algorithm verbatim: in-flight guard → SetMoveFarTo → committed-movement early-out >10yd → stuck check [5yd rule, ≥5 tries, moveStuckTime] → `<pathFinderDis` straight → PathFinder-to-dest w/ progress guard → 2-sample forward-cone fallback → teleport recovery), `MoveWorldObjectTo` (±25% approach jitter, leads moving targets), `MoveRandomNear` (8-attempt water/blocked/unreachable-checked wander). `LastMovementValue` rewritten as AC superset. Config `PlayerBot.PathFinderDis=70`, `MoveStuckTime=90000`, `MaxMovementSearchTime=3`. **API:** `PathGenerator`→`PathFinder`/`PathInfo` (`calculate`/`getPathType`/`getActualEndPosition`/`getPath`/`Length`); `PATHFIND_FARFROMPOLY` absent (SHORTCUT stands in); `rand_norm`→`rand_norm_f`; `Unit::IsMoving` (cap I); `isFrozen` is DEAD → `IsFrozen()`; `Map::GetHeight`; `Player::FindQuestSlot`/`GetQuestSlotQuestId` made public (AC parity).
- **L2 (2026-09-19, DONE + verified 10-bot soak): RPG walking state machine** — AC `NewRpgInfo` weighted-random state machine: bots **WALK** to a grind spot (L1 `MoveFarTo` pathfinding), wander locally (`MoveRandomNear`), rest. New files: `Util/PlayerGrindMgr` (level-band grind-position cache = AC's `sTravelMgr.GetLocsPerLevelCache` vanilla equivalent: startup SQL `creature`⋈`creature_template` rank=0/lvl1-60/maps0-1 ~63k spawns, grid-sampled 1500yd×5lvl band → **1,067 spots live**; `SelectRandomGrindPos` = same map + lvl±3 + ≤2500yd(lo)/<500yd(hi), 50% hi else lo — **AC's exact distances** — + `sMapMgr.FindMap(mapId)->GetHeight`); `Ai/World/Rpg/{RpgBaseAction,RpgActions,RpgTriggers,RpgStrategy}` (`RpgBaseAction:MovementAction` = `RandomChangeStatus` [weighted float roll, not AC's `urand(1,probSum)` truncation] + `CheckRpgStatusAvailable`; `RpgStatusUpdateAction` @11.0 = the driver [IDLE→pick, GO_GRIND→WANDER on arrival <10yd, WANDER/REST→IDLE on `HasStatusPersisted(maxDur*1000)`]; `RpgGoGrindAction`=`MoveFarTo(GetMapId(),grindXYZ)`+`MoveRandomNear(10)` nudge [WALK]; `RpgWanderRandomAction`=`MoveRandomNear`; `RpgStrategy` "rpg" + `RpgStatusTrigger` per-status). Config `rpgStatusProbWeight[]` (GO_GRIND50/WANDER30/REST10/IDLE0/DO_QUEST0), `rpgWander/Rest/DoQuestStatusMaxDuration` (SECONDS; ×1000 for ms `HasStatusPersisted`), `rpgEnabled=false` (default OFF — per-bot `adds rpg` via R6.1c; broad rollout = separate decision). **Verified live:** 300/300, 0 crashes; cache loads 1,067 spots; bots **WALK** (240-1700yd smooth pathfinding, not teleport); state machine **cycles** (`playerbots: rpg <bot> -> rest/go_grind` in info.log); bots **grind** (combat mid-walk); **stuck-recovery teleport** fires when a spot is unreachable (L1 MoveFarTo fallback). **Known AC dynamics (not bugs):** GO_GRIND walks up to 2500yd (AC's exact) so bots can die/get-stuck before arrival → `go_grind→wander` (AC `dis<10`) correct but not always observed for a bot that dies mid-walk; world baseline death ~14%; Rpg (NON_COMBAT) pauses in COMBAT/DEAD engine (expected). **Pitfalls:** `Action::Execute(Event)` BY VALUE; `QueryResult` in `Database/DatabaseEnv.h` (no `QueryResult.h`); `sMapMgr` has NO `GetHeight` → `FindMap(mapId)->GetHeight`; `AI_VALUE` needs `#include "AiObjectContext.h"`; durations in SECONDS → ×1000.
- **L3 (2026-09-19, DONE + verified live): quest DATA layer** — exposes the `PlayerQuestMgr` startup cache to the AI via 4 context values (AC `QuestValues.h` port). `PlayerQuestMgr::Load()` made **unconditional** (was gated by `questEnabled`); **fixed a latent SIGABRT**: the objectives SQL used `quest_template.id` but the PK is **`entry`** (not `id`, same as `creature_template`) → errno 1054 → uncaught → `std::terminate()` (never ran before, Load was gated off); also fixed an **off-by-one** (objectives `objidx` was 1-4 but the C++ `ReqCreatureOrGOId[4]`/`m_creatureOrGOcount[4]` are 0-3). New accessors: `GetFreeQuestLogSlots` / `GetActiveObjectives` (incomplete-quest kill POIs) / `GetActiveTakers` (complete-not-rewarded taker POIs) / `GetNearbyGivers` (live cell scan + `PrepareQuestMenu` + `WorthAccepting`), all same-map+zone+`questPoiMaxDist=1500` (givers `questAcceptRadius=80`), nearest-first. New `Ai/Base/Value/QuestValues.h/.cpp`: `FreeQuestLogSlotValue` ("free quest log slots"), `ActiveQuestGivers/Takers/ObjectivesValue` ("active quest givers/takers/objectives") — vector values return `std::vector<QuestDest>` (nearest-first). Verification hook: `questdata` field in the agent `state` snapshot. **Verified live:** cache loads `5938 taker / 555 objective quests / 2163 giver entries`, 300/300 online, 0 crashes; `questdata` computes — `freeSlots:20` + **`givers:1`** for a lvl-34 bot at a Stormwind giver (cell scan + `WorthAccepting` level/class filter works; quest 1953 Warlock-only [128] correctly not matched by a Priest). takers/objectives = same pattern, exercised end-to-end by L4/L5. **Pitfalls:** `quest_template` PK=`entry`; `creature` PK=`guid` (`creature.id`=entry); `creature_involvedrelation`/`questrelation` cols=`id`+`quest`; `getQuestStatusMap()`/`PrepareQuestMenu` NON-CONST → accessors take `Player*`; `QUEST_OBJECTIVES_COUNT=4` (0-3); low-level bot at an Elwynn giver dies to local mobs → use a safe-town bot for the live givers test.
- **L4/L5 (2026-09-20, DONE + deployed + accept verified live): quest ACTIONS + WIRING** — the do-quest state machine. `RpgBaseAction` gains 8 quest helpers (AC `NewRpgBaseAction` port): `SearchQuestGiverAndAcceptOrReward` (cell-scan nearest "possible rpg target" ≤`questAcceptRadius` with `HasQuestToAcceptOrReward` → `CanInteractWithQuestGiver` → `InteractWithGiverForQuest`, else `MoveWorldObjectTo`; **called from go_grind/wander/do_quest so bots pick up quests while milling in towns**), `HasQuestToAcceptOrReward` (`PrepareQuestMenu` + scan: any COMPLETE+`CanReward` → turn-in, any NONE+`WorthAccepting`+`CanTake`+`CanAdd` → accept), `ChooseGiverToInteract` (overworld cell scan of `PlayerQuestMgr::GetGiverEntries()`), `AcceptQuestAtGiver` (DIRECT `AddQuest` — **not** CMSG replay — guarded by `CanTakeQuest`+`CanAddQuest`), `TurnInQuestAtGiver` (`CompleteQuest` if not COMPLETE → `CanRewardQuest(quest,bestIdx)` → `RewardQuest(quest,bestIdx,giver)`; `CompleteQuest` auto-rewards on `QUEST_FLAGS_AUTO_REWARDED`), `BestRewardIndex` (`StatsWeightCalculator` score of `RewChoiceItemId[]`), `SelectDoQuestQuest` (iterate log, skip `rpgLowPriorityQuest` [new per-bot `std::unordered_set` on `PlayerBotAI`], pick a quest with a valid kill-POI/taker-POI). `PlayerQuestMgr` gains `GetQuestObjectivePoi` (nearest incomplete-kill-objective spawn + objIdx), `GetQuestTakerPoi` (nearest taker POI), `IsWorthAccepting` (public). `RpgDoQuestAction` (new, registered `"rpg do quest"`): `SearchQuestGiverAndAcceptOrReward` → switch on `GetQuestStatus` (INCOMPLETE→`DoIncompleteQuest` [walk nearest kill POI via `MoveFarTo`, arrival `lastReachPOI`, `questNoProgressSeconds` no-kill → `rpgLowPriorityQuest`+abandon], COMPLETE→`DoCompletedQuest` [walk taker POI, turn-in by `SearchQuestGiver`], else→IDLE). `RpgStrategy` adds `"do quest status"` → `NextAction("rpg do quest",3.0f)`. Config weights → AC defaults (DO_QUEST=60 dominant, GO_GRIND=15, WANDER=15, REST=5, IDLE=0). **POI selection = NEAREST (documented simplification; AC picks random).** **Pacing (AC `ForceToWait(5000)` parity):** `PlayerRpgInfo::lastQuestInteract` ms timestamp → `SearchQuestGiver` skips re-interaction for 5s after an accept/turn-in (spaces out rapid-fire menu hits). **Verified live (2026-09-20):** build green, 300/300 online, 0 crashes; **AI-driven ACCEPT PROVEN in a natural scenario** — Inisto (lvl-59 Druid) traveled to Darnassus (R5e travel) + the AI accepted 4 level-matched quests (7792/7798/7799/7800 "A Donation of …: Darnassus") via `SearchQuestGiverAndAcceptOrReward` (quest log shows all in_progress; `questdata` freeSlots=16). **NEXT: full KILL-quest cycle (accept→walk→kill→turn-in)** — the donation quests are item-delivery (no kill POI, so `do_quest` doesn't roll for them); a kill quest is needed to verify the walk-to-POI + `m_creatureOrGOcount` progress + taker turn-in. Blocked by the controlled-test 5-way alignment (level±2 band + class + faction + safe-zone + known objective spawn) for the 10 high-level soak bots; the 300-bot world exercises it naturally (bots travel to level-matched towns + accept kill quests). **Controlled-test lessons:** kill-quest givers are in HOSTILE zones (bot aggro's → dies before the NON_COMBAT accept fires); faction matters (a Horde bot attacks an Alliance giver — the quest giver is faction-hostile to the bot); use a SAFE-town giver OR a tank that survives the aggro. **Pitfalls:** `QuestDef.h` is the `Quest` class (not `Quest.h`); `QUEST_OBJECTIVES_COUNT=4`/`QUEST_REWARD_CHOICES_COUNT=6`; `QuestMenu::GetItem` NON-CONST (bind `QuestMenu&`); `CompleteQuest` auto-rewards on `QUEST_FLAGS_AUTO_REWARDED` (explicit `RewardQuest` only if `CanRewardQuest(quest,bestIdx)` to avoid double-reward); `InteractWithGiverForQuest` must be declared in the .h (not just defined); engine relevance = higher-runs-first (status-update 5.0 before do-quest 3.0 = the timer pattern); `GiverScannerVisitor` is file-scope in PlayerQuestMgr.cpp (RpgBaseAction has its own `RpgGiverScanner`) — no `FindNearestCreature` in this core (cell scan directly).
- **R7 roadmap:** L1 done → L2 done → L3 done → **L4/L5 done (accept verified; kill-cycle next)** → R4 groups/dungeons. WotLK statuses (GoCamp/WanderNpc/TravelFlight/OutdoorPvP) omitted; vanilla dungeons/raids = R4.

### Overnight work (2026-09-19/20, DONE + deployed + verified live)
Five user-directed items, built in one pass + verified on the live 300-bot world:

- **L6: ACSOAP realtime transport (2026-09-19):** ported AC's `ACSoap` — a **gSOAP HTTP server** on a dedicated thread (`SOAP.Enabled`/`SOAP.IP=0.0.0.0`/`SOAP.Port=7878`/`SOAP.MinRank=3`). One web-service method `executeCommand(command)` → `sWorld.QueueCliCommand(new CliCommandHolder(account, sAccountMgr.GetSecurity(account), …))` runs the command at the authenticated account's **REAL** rank (no privilege escalation) + returns the CLI print buffer (the `SendSysMessage`/`PSendSysMessage` output). Auth = HTTP Basic (`soap->userid`/`soap->passwd`) via `sAccountMgr.GetId`+`CheckPassword`+`GetSecurity >= MinRank`. **Vendored gSOAP** into `src/game/ACSoap/gsoap/` (separate static lib, no game PCH, no OpenSSL) so the build stays hermetic. `Master.cpp` starts `ACSoapThread` after the cli thread (bind failure = log + disable, non-fatal). **Verified live:** `curl -u admin:admin -X POST http://127.0.0.1:7878/` with a SOAP envelope → `version` faults (auth + round-trip work); `playerbots pmon reset` → `Performance monitor reset.` (CLI output captured). **Pitfalls:** `CliCommandHolder` uses `std::any callbackArg` (not `void*`); gSOAP needs a `process_message` forward decl + compiles as its own lib (game PCH breaks it); SOAP returns ONLY the CLI print buffer (commands that only `sLog` return empty — e.g. `rndbot stats` → use `pmon reset` for a visible result). **Replaces the file-command queue as the preferred realtime request/response transport** (the file queue `/tmp/agent_cmd.sh` stays as a fallback).

- **Chest/GO looting (2026-09-20):** `AddAllLootAction` (NON_COMBAT "often") now scans **GameObjects** via a `Visit(GameObjectMapType &)` — ready, spawned, loot-templated (`GetLootId()>0`), level-OK chests are added to the `LootObjectStack` (creature corpses unchanged). `OpenLootAction` opens a GO chest via `bot->SendLoot(goGuid, LOOT_CORPSE)` (populates `go->loot`, sets `GO_ACTIVATED`) then the existing store/loot pipeline takes it. **CRITICAL FIX:** the scan must use `Cell::VisitGridObjects` (visits `AllGridObjectTypes` = GameObject/Creature/DynamicObject/Corpse) — the old `WorldTypeMapContainer` visit (AllWorldObjectTypes = Player/Creature/Corpse/Camera) **never called the GO visitor**, so GO looting was dead code. **Verified live:** 5 natural GO loots by 4 bots (entry 180905×4 → item 21174; entry 3662 → item 4536). **Test chest:** custom no-lock chest `900001` (type 3, lootId 2039, `data0`=lock=0, restock 300s) at the level-1 Alliance spawn (Northshire -8774,-184,83) — DB seed (the in-memory instance is at the prior Elwynn test spot until the next restart). **Pitfalls:** chest `data0`=lockId, `data1`=lootId, `data2`=restock, `data9`=level; `GAMEOBJECT_TYPE_CHEST=3`; `gameobject_template`/`gameobject` are SQL in `tw_world` (not DBC); a test chest in a mob nest (e.g. Elwynn Defias) gets a level-1 bot killed → NON_COMBAT loot never fires (use a safe spot OR rely on the natural-loot verification). **v2 (deferred):** locked-chest/lockpick + skill-gated gathering (mining/herbalism/skinning) — level-1 bots lack the profession skills so it's a higher-level feature.

- **Level-1 regen + broad RPG/quest enablement (2026-09-19/20):** `PlayerbotFactory::PickRandomLevel()` now reads `PlayerBot.FactoryLevelMin`/`FactoryLevelMax` (default 1/1) instead of the weighted 1-60 distribution → **all 300 bots regenerated at level 1** (wipe+recreate via `DeleteAllBots=1`→`0`). `PlayerBot.RpgEnabled=1` (rpg strategy added to every bot on login — the L2 walking state machine + L4/L5 do-quest is now the world-wide behavior) + `PlayerBot.QuestEnabled=1` (L3 quest sweep). **Verified:** 300/300 online at level 1, every bot logged `rpg strategy added`, `rpgEnabled=1` + `QuestEnabled=1` in the startup log, 990+ `rpg` status cycles, 0 crashes. **Goal:** low-level kill/collect quests drive natural progression (bots walk + quest from level 1).

- **Idle-relocation sweep REMOVED (2026-09-19, user-directed):** the R5d 180s-idle teleport sweep was **invented** (AC mod-playerbots has no idle-relocation — it walks; teleport is stuck-recovery only). Removed `relocateIdleEnabled`/`relocateIdleSeconds` from config + the sweep block from `PlayerBotMgr::Update`. **Kept:** the **stale-combat breaker** (combat stuck-recovery, legitimate teleport) + the shared `RelocateBotToGrindSpot` helper (now only used by the stale-combat breaker, town-pool via `PlayerTravelMgr::PickDestination`). `ServerFacade` idle-clock methods kept (travel/quest still use them).

**Overnight runtime:** realmd/mangosd up, 300/300 level-1 bots, SOAP live on :7878, natural GO chest looting + rpg quest cycling confirmed, 0 real crashes.
