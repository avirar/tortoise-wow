<!-- Canonical copy: /root/AGENTS.md — edits there, then re-sync. Synced 2026-09-19 (R7 L3 quest DATA layer DONE + verified live: PlayerQuestMgr::Load unconditional + 4 AI context values (free quest log slots / active quest givers/takers/objectives); quest_template PK=entry SIGABRT fix + objidx 0-3 off-by-one fix; questdata state field; cache 5938 taker/555 objective/2163 giver). -->
...
- **R7 roadmap:** L1 done → L2 done → **L3 done** → L4 quest actions (Talk/Acept/TurnIn/Drop/Decline via CMSG replay) → L5 wiring (do-quest state machine: find giver → walk → accept → walk POI → grind (tap) → progress check → walk taker → turnin → next). WotLK statuses (GoCamp/WanderNpc/TravelFlight/OutdoorPvP) omitted; vanilla dungeons/raids = R4.
