/**
 * @file PaladinActions.cpp
 * @brief Paladin class-specific combat action implementations (Turtle WoW 1.18.1)
 */
...
bool CastPaladinSpell(PlayerBotAI* ai, Player* bot, Unit* target, uint32 baseSpellId)
{
    if (!bot || !bot->IsAlive())
        return false;
...
CastHammerOfJusticeAction::CastHammerOfJusticeAction(PlayerBotAI* botAI)
    : Action(botAI, "hammer of justice")
{
}

bool CastHammerOfJusticeAction::Execute([[maybe_unused]] Event event)
{
    Unit* target = GetAiObjectContext()->GetValue<Unit*>("current target")->Get();
    if (!target || !target->IsAlive() || !target->IsInWorld())
        return false;

    return CastPaladinSpell(botAI, bot, target, SPELL_HAMMER_OF_JUSTICE);
}

bool CastHammerOfJusticeAction::isUseful()
{
    return true; // Gated by the "can hammer of justice" trigger
}
